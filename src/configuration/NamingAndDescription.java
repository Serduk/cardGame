package configuration;

/**
 * Here will be insert all naming:
 * Button name,
 * tooltip in menu, subMenu
 *
 * Created by sserdiuk on 7/3/17.
 */
public final class NamingAndDescription {
    public final static String appVersionNum = "0.0.1";

    /*
    * GUI:
    * Text on buttons and tooltips.
    * */
    public final static String gameName = "My Card Game";
    public final static String endTurnButton = "End Turn";
    public final static String endGameButton = "End Game";
    public final static String sendMessageButton = "Send Message";
    public final static String resourceAvailableTultipe = "Resources";
    public final static String parametersTultipe = "Parameters";
    public final static String appVersionTultiope = "App Version";

    /*
    * Text on Buttons
    * */
    public final static String newGame = "New Game";

    /*
    * Error messages
    * And Warning notification
    * */
    public static final String setMainCardsDeckError = "Please set main cards deck first";
    public static final String setMainCardsDeckSuccess = "Main cards deck successfully setup";


}
